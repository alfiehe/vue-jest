export default () => {
  ...
  return {
    ...
    fragmentShader: `
      ...
      float cirlce(vec2 uv)
      {
        //当前像素到中心点的距离
        float dis = distance(uv, center);
        return feather(mask_size.x, mask_size.x, dis*2.);
      }

      float horizontal_ellipse(vec2 uv)
      {
        float c = sqrt(mask_size.x * mask_size.x - mask_size.y*mask_size.y) * 0.5;
        vec2 c0  = center - vec2(c, 0.);
        vec2 c1 = center + vec2(c, 0.);
        float dis = distance(uv, c0) + distance(uv, c1);
        return feather(mask_size.x, mask_size.x, dis);
      }

      float vertical_ellipse(vec2 uv)
      {
        float c = sqrt(mask_size.y*mask_size.y-mask_size.x * mask_size.x) * 0.5;
        vec2 c0  = center - vec2(0., c);
        vec2 c1 = center + vec2(0., c);
        float dis = distance(uv, c0) + distance(uv, c1);
        return feather(mask_size.y, mask_size.y, dis);
      }

      float mask(vec2 pos)
      {
          if( abs(mask_size.x - mask_size.y) < 0.0001)
          {
            return cirlce(pos);
          }
          else if(mask_size.x > mask_size.y)
          {
            return horizontal_ellipse(pos);
          }
          else
          {
            return vertical_ellipse(pos);
          }
      }
